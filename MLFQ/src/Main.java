...
public class Main {
    ...
    static int time = 0;
    static int quantums[] = {0,5,10,1000000000};
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (line != null) {
                ...
                processes.add(new Process(execs, index,1));
                index++;
        ...
        while (completed.size() != processes.size()) {
            ...
            if(quantums[current_process.priority]==counter && current_process.currentExecTime>0 && !current_process.is_io && !current_process.is_wait && !current_process.finished){
                for (Process p:
                        processes) {
                    p.order--;
                }
                System.out.println(current_process.name + " Demoted to Next Queue At Time: " + time);
                current_process.priority++;
                current_process.putToWait(time);
                current_process.exec_times.add(0,current_process.currentExecTime);
                current_process = getNextReady();
                counter = 0;
                if(current_process.exec_times.size()!=0 && current_process.is_wait) {
                    current_process.putToCPU(time);
...
class Process {
    ...
    public Process(List<Integer> executions, int index,int priority) {
    ...
    public void putToWait(int time) {
        ...
        this.order = (priority-1)*25+(Main.processes.size()-Main.IO_Queue.size())*priority;
